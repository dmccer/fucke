...
transByFtp = function (dir, rmtdir, env) {
...
sync = function (dir, rmtdir, env) {
    ...
    if (env.serverName === 'f2e') {
        sftp(env)
    } else {
        transByFtp(dir, rmtdir, env)
    }
};
...
module.exports = function (server, appName, options) {
    ...
    curEnvConfig = _.extend({
        server: {
            host: config.servers[server].host,
            user: config.servers[server].user,
            pass: config.servers[server].pass,
            port: config.servers[server].port
        },
        root: config.servers[server].path,
        appName: appName,
        remotePath: path.join(config.servers[server].path, appName),
        locPath: process.cwd(),
        serverName: server
    });

    if (!curEnvConfig.locPath) {
    ...
    fs.stat(dir, function (err, stats) {
        if (err || !stats.isDirectory()) {
            ...
            console.log(colorized.bright('即将上传：'), dir, NL + DELIMITER_TITLE);
